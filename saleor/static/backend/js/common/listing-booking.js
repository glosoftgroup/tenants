...
var parent = new Vue({
    ...
    methods:{
        toggleView(){
            this.list_grid = !this.list_grid;
            this.grid_list = !this.grid_list;
        },
        processImage(data, full=false){
            if(data){
        ...
        activeWing(id){
            if(id === this.active_wing)
            return 'active';
        },
        getRooms(id){
             this.wings.forEach(item => {
                if (item.id === id) {
                   this.rooms = item.room_wing;
                }
             });
        },
        roomStatus(status){
        ...
        listWings:function(num){
        /* make api request when pagination pages are clicked */
            if(this.date == 'Select date'){
                date = '';
            }else{ date = this.date; }
            this.$http.get('/api/property/wing/')
                .then(function(data){
                    data = JSON.parse(data.bodyText);
                    this.wings = data.results;
                    this.active_wing = data.results[Object.keys(data.results)[0]].id;
                    // populate active wing rooms
                    this.getRooms(this.active_wing);
                }, function(error){
                    console.log(error.statusText);
            });
        },
